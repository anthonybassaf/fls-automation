...
#     # --- STEP 1: Determine reference Z (from gridlines)
#     all_z_values = []
#     for start, end, _ in gridlines or []:
#         all_z_values.extend([start.z, end.z])
#     reference_z = sum(all_z_values) / len(all_z_values) if all_z_values else 0.0

#     # --- STEP 2: Helpers with Z normalization
#     def point_to_tuple(p: Point):
#         return (round(p.x, 4), round(p.y, 4), round(reference_z, 4))
...
#         # Assign room_id and room_name if available
...
#             start=Point(x=to_m(line.start.x), y=to_m(line.start.y), z=reference_z, units="m"),
#             end=Point(x=to_m(line.end.x), y=to_m(line.end.y), z=reference_z, units="m"),
#             units="m",
...
#     return G, wall_lines_2d, reference_z
...
def trim_gridlines(gridlines, wall_lines_2d, doors, grid_dict, spacing=0.5, gap_offset=0.3, reference_z=0.0):
    """
    ...
    def create_door_bridge(door, grid_dict, spacing=0.5, reference_z=0.0, max_projection_factor=2.0):
        m = getattr(door, "transform", None)
        if not m or not hasattr(m, "matrix") or len(m.matrix) < 16:
            return None
    ...
    bridge_edges = []
    for door in doors:
        bridge = create_door_bridge(door, grid_dict, spacing=spacing, reference_z=reference_z)
        if bridge:
...
def add_doors_on_grid(G, doors, reference_z):
...
def add_stairs_on_grid(G, stairs, reference_z):
